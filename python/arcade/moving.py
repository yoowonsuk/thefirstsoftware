...
class Rectangle:
    ...
    def draw(self):
        #draw our rectangle
        arcade.draw_rectangle_filled(self.x,self.y, self.rect_width, self.rect_height,
    ...
    def move(self):
        ...
        self.y += self.new_y
        if self.y < RECTANGLE_HEIGTH // 2:
            self.y = RECTANGLE_HEIGTH // 2
        if self.y > WINDOW_HEIGTH - (RECTANGLE_HEIGTH // 2):
            self.y = WINDOW_HEIGTH- (RECTANGLE_HEIGTH // 2)

     
class Game(arcade.Window):
    def __init__(self, width, height):
        super().__init__(width,height,title="A USER CONTROLLED MOVING RECTANGLE")
...
if __name__ == "__main__":
    obj1 = Game(WINDOW_WIDTH, WINDOW_HEIGTH)
    obj1.setup()
    arcade.run()
