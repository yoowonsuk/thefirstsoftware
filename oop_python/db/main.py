...
class Seat:
    ...
    def is_free(self):
        """Chck in the database if a Seat is taken or not"""
        connection = sqlite3.connect(self.database)
        cursor = connection.cursor()
        cursor.execute("""
    ...
    def occupy(self):
        """Change value of taken in the database from 0 to 1 if Seat if free"""
        if self.is_free():
            connection = sqlite3.connect(self.database)
            connection.execute("""
...
class Ticket:
    ...
    def to_pdf(self):
        ...
        pdf = FPDF(oreientation='P', unit='pt', format='A4')
        pdf.add_page()
        ...
        pdf.set_font(family="Times", style="", size=12)
        pdf.cell(w=0, h=25, txt=str(self.number), border=1, ln=1)
        pdf.cell(w=0, h=5, txt="", border=0, ln=1)
